...
const propsDescs = {
  ...
  noRipple: 'When <code>true</code>, disables the pointer ripple on sortable headers and interactive row cells for this table. By default, ripple follows <code>$waveui.config.ripple</code>.',
  ...
  tableClass: 'A CSS class to apply to the inner <code>&lt;table&gt;</code> element. Accepts the same values as Vue\'s <code>:class</code> binding: a string, an array, or an object.',
  headerClass: 'A CSS class to apply to every <code>&lt;th&gt;</code> header cell. Accepts the same values as Vue\'s <code>:class</code> binding: a string, an array, or an object.',
  itemClass: 'A CSS class to apply to every <code>&lt;tr&gt;</code> item row. Accepts a string, array, object, or a function called with <code>(item, index)</code> that returns a valid Vue class value — useful for per-row conditional styling.',
...
export default {
  ...
  computed: {
    ...
    events () {
      return WTable.emits.reduce((obj, label) => {
