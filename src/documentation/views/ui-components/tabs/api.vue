...
const propsDescs = {
  ...
  itemIdKey: 'Specifies the property on each tab item that holds a stable unique key (default: <code>id</code>). You should set it when you add, remove, or reorder tabs dynamically so selection and transitions stay correct.<br>If missing on an item, Wave UI injects a stable key derived from Vue’s <code>useId()</code> and the tab index so server HTML and client hydration stay in sync (no reliance on internal Vue instance ids).',
  ...
  noRipple: 'When <code>true</code>, disables the pointer ripple on this tabs instance. By default, ripple follows <code>$waveui.config.ripple</code>.'
}
