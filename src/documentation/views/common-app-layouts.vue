<template lang="pug">
main
  title-link.mt4(h1) Common app layouts
  p.
    Wave UI places a #[span.code .w-app] wrapping class at the root of your app.

  alert.mt8.mb2(info).
    By default, the #[span.code w-app] wrapper will have a flex layout with a column direction,
    and a height equal to 100% of the viewport.#[br]
  p.
    ...
    If you prefer to have a display: block layout, you can set the #[span.code d-block] class on the .w-app.

  title-link(h2 slug="setting-app-classes") Setting app classes
  p.
    You can add custom classes to the #[span.code .w-app] element using either the Wave UI config
    or the #[span.code $waveui.setAppClasses()] method.

  w-accordion(expand-icon-rotate90 title-class="pl0")
    w-accordion-item(title="Via Wave UI config")
      p You can set custom classes via the Wave UI configuration on app initialization:
    ...
    w-accordion-item(title="Via $waveui.setAppClasses()")
      p You can also dynamically set app classes at any time during your app's lifecycle:
      ssh-pre.mt4(language="js" label="Javascript" :dark="$store.state.darkMode").
        // Add classes
        ...
        fieldset.app.w-flex.column
          ...
          .w-flex
            ...
            .w-flex.column
              ...
              section.footer Footer
        template(#pug).
          //- Set the .row class on the .w-app.
          aside Nav menu
          w-flex(column)
            header Header
            main.grow Main content
      ...
      example.ma0.grow(content-class="pa0" :blank-codepen="['html', 'pug', 'css']")
        fieldset.app.grow
          ...
          .w-flex
            ...
            section.nav-menu.block Nav menu
        template(#pug).
          //- Set the .d-block class on the .w-app.
          header Header
          w-flex
